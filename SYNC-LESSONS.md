...
5. **Add every origin you'll actually load the app from** to Authorized
   ...
   before even opening the picker. This is exactly the part the user's
   note above is anticipating: **a new project (Hours Ledger) has a new
   origin, so this step has to be redone from scratch**, not copied from
   Money Ledger's client.
...
7. **Known limitation, not a bug to chase**: GIS's silent-reauth path
   (`prompt: ""`) increasingly fails due to third-party-cookie blocking in
   modern browsers — this is a platform trend, not something fixable from
   the app side. Money Ledger's workaround is caching the access token
   itself in `localStorage` (its own key, kept out of synced state, same
   pattern as `DEVICE_ID`) with its real expiry and a small safety margin,
   so a page reload within the token's ~1hr life reuses it with no Google
   round-trip at all. A real refresh token would fix this properly but
   needs the server-side flow, which needs a backend holding a client
   secret — precisely what hard rule 4 forbids. Expect to make the same
   trade for Hours Ledger.
